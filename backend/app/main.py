...
#     # Check if user already exists in the database (using name as a unique identifier)
#     existing_user = user_collection.find_one({"name": user.name})
    
#     # Update or insert user data
#     if existing_user:
#         user_collection.update_one({"name": user.name}, {"$set": user.dict()})
#     else:
...
>>>>>>> 7a849e761c8c3f495fdd0492984627e0c85228ac
from fastapi import FastAPI, Request, HTTPException
from fastapi.middleware.cors import CORSMiddleware
from .calculate_macros import get_nutrition_plan
from .calculate_plan import create_daily_plan
...
async def save_user_settings(user: UserSettings):
    ...
    if existing_user:
        user_collection.update_one({"name": user.name}, {"$set": user.dict()})
        logging.info(f"Updated user settings for {user.name}")
    else:
        user_collection.insert_one(user.dict())
        logging.info(f"Inserted new user settings for {user.name}")
